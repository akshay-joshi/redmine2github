...
try:
    from settings.base import GITHUB_LOGIN, \
        GITHUB_PASSWORD_OR_PERSONAL_ACCESS_TOKEN, GITHUB_TARGET_USERNAME, \
...
class LabelHelper:
    ...
    def __init__(self, label_map_filename=None):
        """The add label to issue seems broken in pygithub3, just use this
        for now"""
    ...
    def make_update_map_labels(self):
        """
        Go through the label make and make sure they all exist, with the
        appropriate colors
        """
        msgt('Match label map names/color to GitHub')

        for label_info in self.label_map.get_label_info_objects():
            msg('\nCheck label: %s %s' % (label_info.github_label_name,
                                          label_info.github_label_color))
            ...
            if req.status_code == 200:
                ...
                if github_label_info.get('color', 'nope') == \
                        label_info.github_label_color:
                    msg('-- color matches!')
                    continue
                ...
                req = requests.patch(label_url, data=json.dumps(data),
                                     auth=self.auth)
                if req.status_code == 200:
                    msg('  Color updated!')
                    msg(req.text)
            ...
            label_url = 'https://api.github.com/repos/%s/%s/labels' % \
                        (GITHUB_TARGET_USERNAME, GITHUB_TARGET_REPOSITORY)
            data = dict(name=label_info.github_label_name,
                        color=label_info.github_label_color)
            req = requests.post(label_url, data=json.dumps(data),
                                auth=self.auth)
            msg(req.text)
            msg(req.status_code)
    ...
    def clear_labels(self, issue_id):
        ...
        label_url = 'https://api.github.com/repos/%s/%s/issues/%s/labels' % \
                    (GITHUB_TARGET_USERNAME, GITHUB_TARGET_REPOSITORY,
                     issue_id)
    ...
    def add_labels_to_issue(self, issue_id, labels=[]):
        msg('Add Labels to Issue.  Issue: [%s] Labels: [%s]' % (issue_id,
                                                                labels))
        ...
        # labels = json.dumps(['invalid', 'bug', 'enhancement',
        # 'duplicate'])#['Bug', 'invalid'])
    ...
    def get_label_from_id_name(self, label_info_dict, key_name=None,
                               label_prefix='', non_formatted=False):
        """
    ...
    def get_label_names_based_on_map(self, redmine_issue_dict):
        """
        We're using the label map, so before using a label, create it with
        the appropriate color.

        If the label doesn't appear in the map, then discard it
        """
        label_names = self.get_label_names(redmine_issue_dict,
                                           non_formatted=True)
        ...
        for name in label_names:
            github_label_name = \
                self.label_map.get_github_label_from_redmine_name(name)
    ...
    def get_label_names(self, redmine_issue_dict, non_formatted=False):
        ...
        # Add status
        status_label_name = self.get_label_from_id_name(redmine_issue_dict,
                                                        'status', 'Status:',
                                                        non_formatted)
        ...
        # Add tracker
        tracker_label_name = self.get_label_from_id_name(redmine_issue_dict,
                                                         'tracker',
                                                         'Tracker:',
                                                         non_formatted)
        ...
        # Add priority
        priority_label_name = self.get_label_from_id_name(redmine_issue_dict,
                                                          'priority',
                                                          'Priority:',
                                                          non_formatted)
        ...
        # Add category
        category_label_name = self.get_label_from_id_name(redmine_issue_dict,
                                                          'category',
                                                          'Category:',
                                                          non_formatted)
        ...
        # Add project
        project_label_name = self.get_label_from_id_name(redmine_issue_dict,
                                                         'project',
                                                         'Project:',
                                                         non_formatted)
        ...
        if custom_fields and len(custom_fields) > 0:
            for cf_dict in custom_fields:
                component_label_name = self.get_label_from_id_name(
                    cf_dict, None, 'Component:', non_formatted)
                if component_label_name and cf_dict.get('value', None) is \
                        not None:
