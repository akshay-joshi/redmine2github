...
# GITHUB_LOGIN, GITHUB_PASSWORD_OR_PERSONAL_ACCESS_TOKEN,
# GITHUB_TARGET_REPOSITORY, GITHUB_TARGET_USERNAME
from settings.base import get_github_auth
...
class MilestoneHelper:
    """
    Certain redmine attributes, such as "fixed_version", will be translated
    into milestones
    """
    ...
    def load_milestone_lookup(self):
        ...
        if not os.path.isfile(self.milestone_mapping_filename):
            msgx('Error: user name file not found: %s' %
        ...
        with open(self.milestone_mapping_filename, 'rU') as csvfile:
            ...
            for row in map_reader:
                ...
                self.milestone_lookup.update(
                    milestone_info.get_label_dict_info())

        msg('Label dict loaded as follows.\nRemember: the "redmine_type" '
            'column in the .csv is ignored by the system--it is only for '
            'user convenience')
        dashes()
        for redmine_name, milestone_info in self.milestone_lookup.items():
            msg('[%s] -> [%s][%s]' % (redmine_name, milestone_info.name,
                                      milestone_info.due_date))
    ...
    def get_create_milestone_number(self, title):
        ...
        If the milestone doesn't exist, then create it and return the new
        number
        """
        if not title:
            return None
    ...
    def get_mile_stone_number(self, title):
        ...
        :returns: int or None.  The milestone number or None, if the
        ...
        kwargs = dict(state='all')
        milestones = self.get_milestones_service().list(**kwargs)
    ...
    def get_milestones_service(self):
        if self.milestone_service is None:
            self.milestone_service = pygithub3.services.issues.Milestones(
                **get_github_auth())
    ...
    def get_create_milestone(self, redmine_issue_dict):
        ...
        #        print(mstone_dict)
        # issue_obj = self.get_github_conn().issues.update(issue_obj.number,
        # mstone_dict)
