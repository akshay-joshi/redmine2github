...
try:
    from settings.base import GITHUB_TARGET_REPOSITORY, \
        GITHUB_TARGET_USERNAME, get_gethub_issue_url
...
class RedmineIssueUpdater:
    """
    For a Redmine project moved to github, update the redmine ticket with
    the github link
    """
    ...
    def __init__(self, redmine_server, redmine_api_key,
                 project_name_or_identifier, issues_dirname,
                 redmine2github_id_map_filename):
        """
        Constructor
        ...
        :param issues_base_directory: str, directory to download the
            redmine issues in JSON format.  Directory will be crated
        """
        ...
        msg('redmine2github_id_map_filename: %s' %
            redmine2github_id_map_filename)
        self.redmine2github_id_map = \
            json.loads(open(redmine2github_id_map_filename, 'rU').read())

        self.redmine_conn = None
        self.redmine_project = None

        self.jinja_env = Environment(loader=PackageLoader(
            'redmine_ticket', 'templates'))
    ...
    def connect_to_redmine(self):
        self.redmine_conn = \
            Redmine(self.redmine_server, key=self.redmine_api_key)
    ...
    def update_tickets(self):
        ...
        for redmine_issue_num in redmine_keys:
            ...
            msgt('(%s) Updating redmine ticket: %s' %
                 (ticket_cnt, redmine_issue_num))
            github_issue_id = \
                self.redmine2github_id_map.get(redmine_issue_num)
            msg('github_issue_id: %s' % github_issue_id)

            fname = redmine_issue_num.zfill(
                RedmineIssueDownloader.ZERO_PADDING_LEVEL) + '.json'
            ...
            redmine_issue_dict = \
                json.loads(open(redmine_issue_fname, 'rU').read())

            github_issue_url = get_gethub_issue_url(github_issue_id)

            template = \
                self.jinja_env.get_template('description_with_github_link.md')
            ...
            updated_record = \
                self.redmine_conn.issue.update(
                    resource_id=int(redmine_issue_num), **update_params)
...
if __name__ == '__main__':
    from settings.base import REDMINE_SERVER, REDMINE_API_KEY, \
        REDMINE_ISSUES_DIRECTORY, REDMINE_TO_GITHUB_MAP_FILE
    ...
    rn = RedmineIssueUpdater(REDMINE_SERVER, REDMINE_API_KEY, 1, issues_dir,
